...
class UserController extends Controller
{
    ...
    {
        $department = $this->department->orderBy('created_at', 'ASC')->get();
    ...
    {
        return DataTables::of($this->table->where('uuid', '!=', Auth::id())->orderBy('created_at', 'desc')->select([
            'uuid',
            'name',
            'username',
            'email',
        ...
        try {
            ...
            if ($store) {
                LogHandler::activity([
                    ...
                    'detail' => 'update data user with username ' . $request->username
                ]);
            }
